...
import requests
from src.data_preprocessing.convert_pdb2npy import convert_pdbs, convert_pdbs_Ab
from src.data_preprocessing.convert_ply2npy import convert_plys
...
def load_protein_npy(pdb_id, data_dir, center=False, single_pdb=False, flexibility=False, binary = False, atomic = False, itsflexible = False):
    ...
    if flexibility:
        if atomic:
            ...
            atom_flex = 100*tensor(np.load(data_dir / (pdb_id + "_itsflexible.npy")))
        else:
...
def load_protein_pair(pdb_id, data_dir,single_pdb=False, flexibility=False, binary = False, atomic = False, itsflexible = False):
    """Loads a protein surface mesh and its features"""
    pspl = pdb_id.split("_")

    try:
        p1_id = pspl[0] + "_" + pspl[1]
        p1 = load_protein_npy(p1_id, data_dir, center=False, single_pdb=single_pdb, flexibility = flexibility, binary = binary, atomic = atomic, itsflexible = itsflexible)
    ...
    try:
        ...
        p2 = load_protein_npy(p2_id, data_dir, center=False, single_pdb=single_pdb, flexibility = flexibility, binary = binary, atomic = atomic, itsflexible = itsflexible)
    except FileNotFoundError:
...
class ProteinPairsSurfaces(InMemoryDataset):
    ...
    def __init__(self, root, ppi=False, train=True, transform=None, pre_transform=None, flexibility=False, antibody = False, crossval = False, subset = 1,  mix = False, binary = False, atomic = False, ab = False, itsflexible = False):
        self.ppi = ppi
    ...
    def process(self):
        ...
        if self.ppi: #search
            if self.antibody: #Ab_Ag
                lists_dir = Path("dataset_list/search/Ab_Ag")
                csv_files = Path("dataset_list/site/cdr3_extracted_gep.csv")
            else:
                lists_dir = Path("dataset_list/search/PPI")
                csv_files = None
                pass
        else: # site   
            lists_dir = Path("dataset_list/site")
            csv_files = Path("dataset_list/site/cdr3_extracted.csv")
        ...
        if not protein_dir.exists():
            ...
            if self.antibody:
                convert_pdbs_Ab(pdb_dir,protein_dir, list_names = pairs_list, atomic = self.atomic, itsflexible=self.itsflexible, csv_files = csv_files)
            else:
                convert_pdbs(pdb_dir,protein_dir, list_names = pairs_list, atomic = self.atomic, csv_files = csv_files)
        ...
        with open(lists_dir / f"training_{prot}.txt") as f_tr, open(
            lists_dir / f"validation_{prot}.txt"
        ) as f_ts:
        ...
        for p in training_pairs_list:
            try:
                protein_pair = load_protein_pair(p, protein_dir, flexibility=self.flexibility, binary = self.binary, atomic = self.atomic, itsflexible = self.itsflexible)
            except FileNotFoundError:
                continue
            except ValueError:
                continue
